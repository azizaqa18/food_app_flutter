...
class CartRepository {
  ...
  ValueNotifier<List<CartItem>> cartNotifier =
      ValueNotifier<List<CartItem>>([]);

  StreamSubscription? _cartSnapShotSubscription;

  Future<Either<ErrorHandler, CartItem>> getCartItem(
      String uid, String foodId) async {
    try {
      final cartItem =
          cartNotifier.value.firstWhere((item) => item.foodId == foodId);
      return Right(cartItem);
    } catch (e) {
      try {
        final _cartSnapshot =
            await _firestoreUser.doc(uid).collection('cart').doc(foodId).get();
        if (_cartSnapshot.exists) {
          fodaPrint(_cartSnapshot.data());
          return Right(
              CartItem.fromMap(_cartSnapshot.data() as Map<String, dynamic>));
        } else {
  ...
  Future<Either<ErrorHandler, List<CartItem>>> getCart(String uid) async {
    _clearCart();
    try {
      _cartSnapShotSubscription = _firestoreUser
          .doc(uid)
          .collection('cart')
          .snapshots()
          .listen(_listenToCart);
    } on FirebaseException catch (e) {
      return Left(ErrorHandler(message: e.message ?? ''));
    }
  ...
  Future<Either<ErrorHandler, bool>> addOrRemoveFoodFromCart(
      String uid, Food food,
      {bool isAdding = true}) async {
    try {
      final dateNow = DateTime.now().toUtc().millisecondsSinceEpoch;

      final cartDocRef =
          _firestoreUser.doc(uid).collection('cart').doc(food.id);
      final foodExistInCart = cartNotifier.value
          .where((cartItem) => cartItem.foodId == food.id)
          .isNotEmpty;

      if (foodExistInCart) {
        final oldItem = cartNotifier.value
            .firstWhere((cartItem) => cartItem.foodId == food.id);
  ...
  Future<Either<ErrorHandler, bool>> removeFoodFromCart(
      String uid, Food food) async {
    try {
      ...
      final foodExistInCart = cartNotifier.value
          ...
          .isNotEmpty;
      if (foodExistInCart) {
        final index = cartNotifier.value
            .indexWhere((element) => element.foodId == food.id);
  ...
  Future<Either<ErrorHandler, bool>> clearItemsFromCart(String uid) async {
    try {
      ...
      for (CartItem cartItem in cartNotifier.value) {
        deleteCartItemBatch.delete(
            _firestoreUser.doc(uid).collection('cart').doc(cartItem.foodId));
      }
  ...
  void _listenToCart(QuerySnapshot<Map<String, dynamic>> snappshot) {
    for (final doc in snappshot.docs) {
      ...
      final index = cartNotifier.value
          .indexWhere((element) => element.foodId == cartItem.foodId);
      if (index != -1) {
        cartNotifier.value.removeAt(index);
